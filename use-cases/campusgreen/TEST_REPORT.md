...
### Tier 2 — LLM-dependent (conversational, same scenarios) · 13 gated tests

`demo_test.py` drives the real OpenAI-backed agent through Agent Kernel's built-in
test harness (`Test("demo.py")`), and `security_test.py` adds two real prompt-injection
probes. These are skipped unless `OPENAI_API_KEY` is set **and** the platform can
import `agentkernel.cli` (its `readline` dependency makes it unavailable on stock
Windows). In CI with the key and a POSIX runner they execute; here they are the 13
skips. Their expectations are checked against the same truths as their Tier-1 twins,
so crossing either tier verifies the same contract.

## Coverage by file

| File | Tests | What it verifies |
| --- | --- | --- |
| `tool_test.py` | 100 | Full unit matrix for all six tools: location lookup (known/alias/id/case/unknown/empty/non-string), create (all 7 categories, ID sequencing, persistence, distinct duplicate IDs, validation codes), get (existing/nonexistent/malformed/mixed-case), update (**36-case lifecycle state machine** + terminal CLOSED + resolve-then-close + no-op same-status), notify (success record, unknown team/issue/type, no-write-on-failure), sustainability reports (seed counts for all periods, filters, validation, malformed-trends fallback), non-string torture of every parameter, the per-session `active_issue_id` cache (written and isolated across sessions), no-write-on-failure file checks, and a 200-create performance smoke test |
