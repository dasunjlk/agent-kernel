# CampusGreen Reliability & Test Report (Phase 7 + Phase 8 + Phase 9A)
...
| Collected | 289 |
| **Passed** | **273** |
| Skipped | 16 (LLM-dependent; see [gating](#gating-llm-only-tests)) |
| Failures | 0 |
| Duration | ~10–15 s |
...
### Tier 1 — Deterministic and runnable (no API key, no network) · 273 tests

Every Tier-1 test drives the **real application path**: the real `AgentTelegramRequestHandler`
...
- the Telegram boundary routes, isolates sessions, and returns replies correctly;
...
### Tier 2 — LLM-dependent (conversational, same scenarios) · 16 gated tests
...
and `security_test.py` adds two real prompt-injection probes, plus `groq_readiness_test.py`
adds a live Groq connection probe. These are skipped
unless `OPENAI_API_KEY`/`GROQ_API_KEY` is set **and** the platform can import `agentkernel.cli`
(its `readline` dependency makes it unavailable on stock Windows). In CI with the
key and a POSIX runner they execute; here they are the 16 skips. Their expectations
...
| `integration_test.py` | 21 | Offline Telegram boundary: routing, per-sender session isolation, reply send-back, missing-agent and error mapping, `/start` and `/help` command handling, normalization, duplicate `update_id`/`message_id` dedup, then **real-tool workflows through the handler**: report chain, unknown location, status from stored data, escalation, truthful tool failure, *partial failure (created-but-not-notified)*, *duplicate reports*, *multi-turn status resolution* |
...
| `e2e_scenario_test.py` | 1 | The 11-step competition scenario end to end through the Telegram handler — report → follow-up status → analytics → second (different-category) report → "was the team notified?" → status by ID → escalation → escalation notification recorded → post-escalation status → truthful partial failure → **final persisted-state audit on disk** |
...
| Obscure startup failure | `server.py` validates required env vars and prints **names only** before `SystemExit(2)`; `validate_config` is importable and tested | `server.py`, `security_test.py` |
...
| Inspecting the data behind a plan (Phase 8) | New `search_issues` tool lists the **actual recorded tickets** (compact, no full history) filtered by category/status/location with a `total_matches` count and a documented, deterministic sort (priority rank, then newest) | `tool.py`, `tool_test.py` |
| Plans that are evidence, not fiction (Phase 8) | `agent.py` ACTION PLANNING block instructs the agent to separate evidence (counts, open status, real ticket IDs/locations) from recommendations, decline cost/savings/forecast questions the data can't compute, and **never act inside a plan** | `agent.py`, `test_helpers.py`, `action_planning_test.py` |
| Acting on a plan (Phase 8) | Only an explicit user request ("escalate the top …") triggers `update_issue` + `notify_team`; plan→why→act scenarios are verified on disk | `test_helpers.py`, `action_planning_test.py` |
| Telegram Messaging Migration (Phase 9A) | Replaced WhatsApp with Telegram: `telegram_handler.py` shim with normalization, `/start`, duplicate event handling, and long polling | `telegram_handler.py`, `server.py`, `integration_demo.py`, `integration_test.py` |
...
- **No deduplication.** V1 deliberately creates a new ticket for each report, even an
  ...
  so the behavior is explicit, not accidental.
...
- **Windows CLI caveat.** `agentkernel.cli` imports the Unix-only `readline`, so `demo.py`
  (CLI) cannot start on stock Windows. The same agent is fully covered by the Telegram path
